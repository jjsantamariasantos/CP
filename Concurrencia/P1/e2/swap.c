...
void *swap(void *ptr)
{
    ...
    while (args ->iterations > 0)
    {
        ...
        if (i < j) {
            pthread_mutex_lock(&args->mutex2[i]);
            pthread_mutex_lock(&args->mutex2[j]);
        } else {
            pthread_mutex_lock(&args->mutex2[j]);
            pthread_mutex_lock(&args->mutex2[i]);
        }
        ...
        pthread_mutex_unlock(&args->mutex2[j]);
        pthread_mutex_unlock(&args->mutex2[i]);
    }
...
void start_threads(struct options opt)
{
    ...
    pthread_mutex_t *aux = malloc(sizeof(pthread_mutex_t) * opt.buffer_size);
    
    for(i = 0; i < opt.buffer_size; i++) {
        if(pthread_mutex_init(&aux[i], NULL) != 0) {
