 ...
 * @param iterations Número de iteraciones que tienen que realizarse en el búfer.
 * @param mutexIterations Mutex usado para proteger la variable iterations.
 */
struct buffer
{
    int *data;
 ...
 * @brief Estructura que contiene la información del hilo que se usará para impresión.
 *
 * @param thread_id Identificador del hilo devuelto por pthread_create().
 */
struct printThreadInfo
...
void *swap(void *ptr)
{
    ...
    while (1)
    {
        ...
        pthread_mutex_lock(&args->buffer->mutexIterations);
        if (args->buffer->iterations <= 0)
        {
            
            pthread_mutex_unlock(&args->buffer->mutexIterations);
            break;
        }
...
void start_threads(struct options opt)
{
    ...
    if (pthread_mutex_init(&buffer.mutexIterations, NULL) != 0)
    {
    ...
    if (finished == NULL || threads == NULL || args == NULL || bufferMutex == NULL || printMutex == NULL)
    {
    ...
    *finished = true;
    ...
    pthread_mutex_destroy(&buffer.mutexIterations);
    // Print the buffer
    printf("Buffer after:  ");
